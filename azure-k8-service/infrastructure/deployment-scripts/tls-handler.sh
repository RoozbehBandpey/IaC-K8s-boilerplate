...
if [[ $secretName == *"$secrets"* ]]; then
    ...
    openssl req -x509 -nodes -days 365 -newkey rsa:2048 -out $secretName'.crt' -keyout $secretName'.key' -subj "/CN=mmc-aks-dev-dns-48b692a2.hcp.koreacentral.azmk8s.io/O=$secretName"
